...
public:
    LLStats(const LLStats & rhs) : vocab_(rhs.vocab_), words_(rhs.words_), unk_(rhs.unk_), correct_(rhs.correct_), loss_(rhs.loss_), is_likelihood_(rhs.is_likelihood_) { }
    LLStats(int vocab=0) : vocab_(vocab), words_(0), unk_(0), correct_(0), loss_(0.0), is_likelihood_(true) { }
    ...
    friend bool operator<(const LLStats& l, const LLStats& r)
    {
      return l.loss_ < r.loss_;
    }

    friend std::ostream &operator<<( std::ostream &output, const LLStats &D ) {
        output << D.loss_;
        return output;
    }
