...
public:
    ...
    LinearEncoder(int vocab_size, int wordrep_size,
             ...
             dynet::Model & model, std::string enc_type="seq",
	     bool update_chs_latt_temp=false, dynet::real chs_latt_temp=0.0,
	     bool update_frg_latt_temp=false, dynet::real frg_latt_temp=0.0);
    ~LinearEncoder() { }
    ...
    dynet::expr::Expression BuildSentGraphLattice(const Sentence & sent, bool add, bool train, dynet::ComputationGraph & cg);
    dynet::expr::Expression BuildSentGraphWords(const Sentence & sent, bool add, bool train, dynet::ComputationGraph & cg);
    dynet::expr::Expression BuildSentGraph(const std::vector<Sentence> & sent, bool add, bool train, dynet::ComputationGraph & cg);
    dynet::expr::Expression BuildSentGraphLattice(const std::vector<Sentence> & sent, bool add, bool train, dynet::ComputationGraph & cg);
    dynet::expr::Expression BuildSentGraphWords(const std::vector<Sentence> & sent, bool add, bool train, dynet::ComputationGraph & cg);
...
protected:
    ...
    // helper for building computation graphs
    dynet::expr::Expression word_repr(dynet::ComputationGraph & cg, const Sentence & sent, int pos);
    dynet::expr::Expression word_repr(dynet::ComputationGraph & cg, const std::vector<Sentence> & sent, int pos);
    ...
    dynet::real chs_latt_temp_, frg_latt_temp_;
    bool update_chs_latt_temp_, update_frg_latt_temp_;

    // Pointers to the parameters
    dynet::LookupParameter p_wr_W_; // Wordrep weights

    // The RNN builder
    BuilderPtr builder_chain_;
    ...
    int word_states_pos;
    void add_word_state_or_skip(dynet::expr::Expression & i_h_t, int pos, bool reverse);
    size_t reset_word_states(const Sentence & sent, bool add);
    size_t reset_word_states(const std::vector<Sentence> & sent, bool add);
