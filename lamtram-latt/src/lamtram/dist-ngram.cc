  ...
  for(size_t i = 0; i < sent.size(); i++) {
    ...
    for(int j = ctxt_pos_.size(); j >= 0; j--) {
      if(j == 0) {
        ...
        ngram.insert_word_begin(get_sym(sent, i-ctxt_pos_[j-1]));
      }
    }
  }
}
...
void DistNgram::calc_ctxt_feats(const Sentence & ctxt, float* feats_out) const {
  ...
  for(int j = ctxt_pos_.size(); j >= 0; j--) {
    ...
    if(j != 0)
      ngram.insert_word_begin(ctxt[ctxt.size()-ctxt_pos_[j-1]]);
  }
}
...
void DistNgram::calc_word_dists(const Sentence & ngram,
  ...
  float base_prob = (*ngram.get_word_ids().rbegin() != UNK_ID ? 1.0 : unk_prob);
  Sentence this_ngram(1, *ngram.get_word_ids().rbegin()), this_ctxt;
  for(int j = ctxt_pos_.size(); j >= 0; j--) {
    ...
    if(ngram_it == mapping_.end()) {
      ...
      if(context_it == mapping_.end())
        THROW_ERROR("ngram ("<<this_ngram.get_word_ids()<<") exists but ctxt (" << this_ctxt.get_word_ids() << ") doesn't");
    ...
    if(j != 0) {
      this_ngram.insert_word_begin(ngram[ngram.size()-ctxt_pos_[j-1]-1]);
      this_ctxt.insert_word_begin(ngram[ngram.size()-ctxt_pos_[j-1]-1]);
    }
  }
...
void DistNgram::calc_all_word_dists(const Sentence & ctxt_ngram,
  ...
  for(int j = ctxt_pos_.size(); j >= 0; j--) {
    ...
    for(int wid = 0; wid < vocab_size; wid++) {
      this_ngram[this_ngram.size()-1] = wid;
//      *this_ngram.get_word_ids().rbegin() = wid;
      auto ngram_it = mapping_.find(this_ngram);
      if(ngram_it != mapping_.end()) {
        int value = (j == 0 ? ngram_it->second : ctxt_cnts_[ngram_it->second].first);
        if(smoothing_ == SMOOTH_MABS || smoothing_ == SMOOTH_MKN) {
    ...
    if(j != 0) {
      this_ngram.insert_word_begin(ctxt_ngram[ctxt_ngram.size()-ctxt_pos_[j-1]]);
      this_ctxt.insert_word_begin(ctxt_ngram[ctxt_ngram.size()-ctxt_pos_[j-1]]);
    }
  }
  // Convert into heuristics if necessary
  if(heuristics_) {
  ...
  // Get the mapping
  {
    ...
    for(int i = 0; i < size; i++) {
      ...
      if(mapping_.find(ngram) != mapping_.end())
        THROW_ERROR("Found duplicate entry for ngram " << ngram.get_word_ids() << " at: " << line << endl);
