...
int Lamtram::SequenceOperation(const boost::program_options::variables_map & vm) {
  ...
  for(string & infile : infiles) {
    ...
    if(eqpos == string::npos)
      THROW_ERROR("Bad model type. Must specify encdec=, encatt=, latatt=, featatt=, or nlm= before model name." << endl << infile);
...
//      if(vm["operation"].as<std::string>() != "gen") THROW_ERROR("currently, only 'gen' operation supported for latatt");
      EncoderAttentional * tm = ModelUtils::LoadBilingualModel<EncoderAttentional>(file, mod_temp, vocab_src_temp, vocab_trg_temp);
      encatts.push_back(shared_ptr<EncoderAttentional>(tm));
    } else if(type == "featatt") {
//      if(vm["operation"].as<std::string>() != "gen") THROW_ERROR("currently, only 'gen' operation supported for featatt");
      EncoderAttentional * tm = ModelUtils::LoadBilingualModel<EncoderAttentional>(file, mod_temp, vocab_src_temp, vocab_trg_temp);
      encatts.push_back(shared_ptr<EncoderAttentional>(tm));
    } else if(type == "nlm") {
      ...
      THROW_ERROR("Unknown model type: " << type);
    }
    // Sanity check
  ...
  std::string src_filetype;
  istream* src_in = NULL;
  if(encdecs.size() + encatts.size() > 0) {
    std::string src_in_path, filetype;
    GetFileNameAndType(vm["src_in"].as<std::string>(), src_in_path, src_filetype, "txt");
    if (src_in_path == "-") {
...
//        sent_src = ParseAsWords(*vocab_src, line, false);
    ...
    while(getline(cin, line)) { 
      ...
      sent_trg = ParseAsWords(*vocab_trg, columns[1], true);
