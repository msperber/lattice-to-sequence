...
ExternAttentional::ExternAttentional(const std::vector<LinearEncoderPtr> & encoders,
                   ...
                   dynet::Model & mod,
		   const bool update_att_latt_temp, const dynet::real att_latt_temp)
    : ExternCalculator(0), encoders_(encoders),
      ...
      cur_sentence_(NULL), attentional_lat_temperature_(att_latt_temp), update_attentional_lat_temperature_(update_att_latt_temp) {
  ...
  if(update_attentional_lat_temperature_){
    dynet::ParameterInitConst pi_const1(attentional_lat_temperature_);
    p_att_lat_temp_ = mod.add_parameters({1}, pi_const1);
  }
...
void ExternAttentional::NewGraph(dynet::ComputationGraph & cg) {
  ...
  if(update_attentional_lat_temperature_){
    i_att_lat_temp_ = parameter(cg, p_att_lat_temp_);
  }
  curr_graph_ = &cg;
}
...
ExternAttentional* ExternAttentional::Read(std::istream & in, const DictPtr & vocab_src, const DictPtr & vocab_trg, dynet::Model & model) {
  ...
  string version_id, attention_type, attention_hist = "none", lex_type = "none", line, att_lat_temp = "0.0";
  bool update_att_lat_temp = false;
  if(!getline(in, line))
  ...
  if(version_id == "extatt_002") {
    ...
    iss >> num_encoders >> attention_type >> attention_hist >> lex_type >> state_size >> update_att_lat_temp >> att_lat_temp;
  } else {
    THROW_ERROR("Expecting a ExternAttentional of version extatt_002-extatt_005, but got something different:" << endl << line);
  }
  vector<LinearEncoderPtr> encoders;
  while(num_encoders-- > 0)
    encoders.push_back(LinearEncoderPtr(LinearEncoder::Read(in, model)));
  return new ExternAttentional(encoders, attention_type, attention_hist, state_size, lex_type, vocab_src, vocab_trg, model, update_att_lat_temp, boost::lexical_cast<dynet::real>(att_lat_temp));
}
void ExternAttentional::Write(std::ostream & out) {
  out << "extatt_005 " << encoders_.size() << " " << attention_type_ << " " << attention_hist_ << " " << lex_type_ << " " << state_size_ << " " << update_attentional_lat_temperature_ << " " << attentional_lat_temperature_ << endl;
...
void ExternAttentional::InitializeSentence(
  ...
  cur_sentence_single_.clear();
  cur_sentence_single_.push_back(sent_src);
  cur_sentence_ = &cur_sentence_single_;
...
void ExternAttentional::InitializeSentence(
  ...
  cur_sentence_multi_ = &sent_src;
  cur_sentence_ = cur_sentence_multi_;
...
dynet::expr::Expression ExternAttentional::CreateContext(
  ...
  // weights biasing the attention
  if((*cur_sentence_)[0].has_scores()) {
    const unsigned num_sent = cur_sentence_->size();
    dynet::expr::Expression i_ww;
    vector<dynet::real> word_weights = vector<dynet::real>(num_sent * sent_len_);

    for(size_t i = 0; i < num_sent; i++){
      for(size_t j = 0; j < sent_len_; j++){
  	const unsigned array_index = j*num_sent+i;
  	float weight = j < (*cur_sentence_)[i].size() ? (*cur_sentence_)[i].get_score_as_prob(j, 1) : 1;
  	if(update_attentional_lat_temperature_){
          word_weights[array_index] = log(weight);
  	} else {
          word_weights[array_index] = attentional_lat_temperature_ * log(weight);
  	}
      }
    }
    i_ww = input(cg, dynet::Dim({(unsigned int)sent_len_},num_sent), word_weights);
    if(update_attentional_lat_temperature_){
      i_ww = i_ww * i_att_lat_temp_;
    }
    i_e = i_e + i_ww;
  }
  if(GlobalVars::verbose >= 3) {
    if(update_attentional_lat_temperature_){
      cerr << "Attention Temperature: " << i_att_lat_temp_.value() << "\n";
    }
  }
...
EncoderAttentional* EncoderAttentional::Read(const DictPtr & vocab_src, const DictPtr & vocab_trg, std::istream & in, dynet::Model & model) {
  ...
  if(version_id == "encatt_001"){
    // pass
  } else if(version_id == "encatt_002"){
    float temperature_legacy;
    iss >> temperature_legacy;
  } else {
    THROW_ERROR("Expecting a EncoderAttentional of version encatt_001..002, but got something different:" << endl << line);
  }
  ...
  EncoderAttentional * ret = new EncoderAttentional(extern_calc, decoder, model);
  return ret;
}
