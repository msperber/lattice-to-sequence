  ...
  for(auto t : boost::irange(0, slen+1)) {
    ...
    if(samp_dist(*dynet::rndeng)) {
      ...
      for(size_t i = 0; i < ngrams.size(); i++) {
        words[i] = ngrams[i].get_last_wordid();
        ctxts[i].resize_words(ctxts[i].size()-1);
      }
      dynet::expr::Expression i_prob = (
        cache_ids.size() ?
        softmax_->CalcProbCache(i_h_t, i_prior, my_cache.get_word_ids(), ctxts, train) :
    ...
    // Otherwise, create the correct
      ...
      i_err = (
        cache_ids.size() ?
        softmax_->CalcLossCache(i_h_t, i_prior, my_cache.get_word_ids(), ngrams, train) :
