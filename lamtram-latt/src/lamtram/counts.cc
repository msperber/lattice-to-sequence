...
void Counts::calc_word_dists(const Sentence & ngram,
  ...
  Sentence ctxt = ngram; ctxt.resize_words(ctxt.size() - 1);
  WordId wid = *ngram.get_word_ids().rbegin();
  auto it = cnts_.find(ctxt);
  if(it == cnts_.end()) {
...
void Counts::read(DictPtr dict, std::istream & in) {
  ...
  while(getline(in, line)) {
    ...
    assert(strs.size() == 2);
    ContextCountsPtr & ptr = cnts_[ParseAsWords(*dict, strs[0], false)];
    assert(ptr.get() == NULL);
