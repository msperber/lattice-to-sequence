...
PyramidLSTMBuilder::PyramidLSTMBuilder(unsigned layers,
                         unsigned input_dim,
                         unsigned hidden_dim,
...
void PyramidLSTMBuilder::new_graph_impl(ComputationGraph& cg){
  param_vars.clear();
...
// layout: 0..layers = c
//         layers+1..2*layers = h
void PyramidLSTMBuilder::start_new_sequence_impl(const vector<Expression>& hinit) {
  h.clear();
  c.clear();
  if (hinit.size() > 0) {
...
unsigned PyramidLSTMBuilder::num_outputs_for_inputs(unsigned input_len){
  return input_len / pow(2, layers-1);
}

unsigned PyramidLSTMBuilder::pyramid_height(unsigned t){
  unsigned m = 1;
  unsigned l = 1;
  while(t % (m*2) == m*2-1 and l < layers){
    m *= 2;
    l += 1;
  }
  return l;
...
Expression PyramidLSTMBuilder::set_h_impl(int prev, const vector<Expression>& h_new) {
...
Expression PyramidLSTMBuilder::set_s_impl(int prev, const std::vector<Expression>& s_new) {
...
Expression PyramidLSTMBuilder::add_input_impl(int prev, const Expression& x) {
  const unsigned cur_step = h.size();
  assert(prev==cur_step-1); // the pyramid doesn't support non-consecutive conditioning

  const unsigned layers_cur_step = pyramid_height(cur_step);

  h.push_back(vector<Expression>(layers_cur_step));
  c.push_back(vector<Expression>(layers_cur_step));
  vector<Expression>& ht = h.back();
  vector<Expression>& ct = c.back();
  Expression in = x;
  for (unsigned i = 0; i < layers_cur_step; ++i) {
    int prev_in_layer = prev;
    while(prev_in_layer>=0 && h[prev_in_layer].size()<=i){
      prev_in_layer -= 1;
    }
    const vector<Expression>& vars = param_vars[i];
    Expression i_h_tm1, i_c_tm1;
    bool has_prev_state = (prev_in_layer >= 0 || has_initial_state);
    if (prev_in_layer < 0) {
      ...
      i_h_tm1 = h[prev_in_layer][i];
      i_c_tm1 = c[prev_in_layer][i];
    }
...
void PyramidLSTMBuilder::copy(const RNNBuilder & rnn) {
  const PyramidLSTMBuilder & rnn_lstm = (const PyramidLSTMBuilder&)rnn;
...
void PyramidLSTMBuilder::save_parameters_pretraining(const string& fname) const {
  cerr << "Writing LSTM parameters to " << fname << endl;
  ofstream of(fname);
  assert(of);
  boost::archive::binary_oarchive oa(of);
  std::string id = "PyramidLSTMBuilder:params";
  oa << id;
  oa << layers;
  for (unsigned i = 0; i < layers; ++i) {
    for (auto p : params[i]) {
...
void PyramidLSTMBuilder::load_parameters_pretraining(const string& fname) {
  ...
  std::string id;
  ia >> id;
  if (id != "PyramidLSTMBuilder:params") {
    cerr << "Bad id read\n";
    abort();
  }
  unsigned l = 0;
...
void PyramidLSTMBuilder::serialize(Archive& ar, const unsigned int) {
  ar & boost::serialization::base_object<RNNBuilder>(*this);
  ar & params;
  ar & layers;
  ar & dropout_rate;
}
DYNET_SERIALIZE_IMPL(PyramidLSTMBuilder);
