#ifndef DYNET_LATTICE_LSTM_H_
#define DYNET_LATTICE_LSTM_H_
...
struct LatticeLSTMBuilder : public dynet::LSTMBuilder {
  LatticeLSTMBuilder() = default;
  explicit LatticeLSTMBuilder(unsigned layers,
                              unsigned input_dim,
                              unsigned hidden_dim,
                              Model* model,
 ...
 public:
  Expression add_input_multichild(const std::vector<int> prevs, const Expression& x, const std::vector<Expression> scores = std::vector<Expression>());
