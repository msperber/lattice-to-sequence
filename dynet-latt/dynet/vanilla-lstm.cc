...
namespace dynet {
  ...
  /*
   * this is derived from lstm.cc, but removes the peepholes and the coupling of input/forget gates.
   */
...
//    Parameter p_bf = model->add_parameters({hidden_dim});
    ParameterInitConst pi_bf(1.f);
    Parameter p_bf = model->add_parameters({hidden_dim}, pi_bf); // initialize forget gates to 1 as recommended by http://www.jmlr.org/proceedings/papers/v37/jozefowicz15.pdf
    ...
    vector<Parameter> ps = {p_x2i, p_h2i, p_bi, p_x2o, p_h2o, p_bo, p_x2f, p_h2f, p_bf, p_x2c, p_h2c, p_bc};
    params.push_back(ps);
  }  // layers
  dropout_rate = 0.f;
}
...
void VanillaLSTMBuilder::new_graph_impl(ComputationGraph& cg) {
  ...
  for (unsigned i = 0; i < layers; ++i) {
    ...
    vector<Expression> vars = {i_x2i, i_h2i, i_bi, i_x2o, i_h2o, i_bo, i_x2f, i_h2f, i_bf, i_x2c, i_h2c, i_bc};
